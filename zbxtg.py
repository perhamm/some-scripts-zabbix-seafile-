...
class TelegramAPI:
    ...
    def http_get(self, url):
        answer = requests.get(url, proxies=dict(http='socks5h://аывавваываываываываыва',https='socks5h://аывавваываываываываыва'), verify=False)
    ...
    def __init__(self, key):
        self.debug = False
        self.key = key
        self.proxies = {'https': "socks5h://аывавваываываываываыва"}
    ...
    def get_updates(self):
        ...
        if self.debug:
            print_message(url)
        answer = requests.post(url, params=params, proxies=dict(http='socks5h://аывавваываываываываыва',https='socks5h://аывавваываываываываыва'), verify=False)
        self.result = answer.json()
        if self.debug:
    ...
    def send_photo(self, to, message, path):
        ...
        answer = requests.post(url, params=params, files=files, proxies=dict(http='socks5h://аывавваываываываываыва',https='socks5h://аывавваываываываываыва'), verify=False)
        self.result = answer.json()
        self.ok_update()
        return self.result
...
class ZabbixAPI:
    ...
    def api_test(self):
        ...
        api = requests.post(api_url, data=api_data, proxies=self.proxies, headers=headers, verify=False)
        return api.text
...
class Maps:
    ...
    def get_coordinates_by_address(self, address):
        ...
        url = url_api
        answer = requests.get(url, proxies=self.proxies, verify=False)
        result = answer.json()
        try:
...
def external_image_get(url, tmp_dir, timeout=6):
    ...
    try:
        answer = requests.get(url, timeout=timeout, verify=False)
