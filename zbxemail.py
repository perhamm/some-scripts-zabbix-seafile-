...
try:
	zapi.session.auth = (api_user, api_pass)
...
class SendEmail(object):
	...
	def send(self):
		...
		msg['To'] = self.mail_to
		if self.mail_priority == 1:
			msg['X-Priority'] = '1'
		if self.mail_priority == 5:
			msg['X-Priority'] = '5'
...
class ZabbixImage(object):
	...
	def graph_get(self, itemid, period, triggername, width, height, problem, hostid, event, trigger):
		...
		for i in range(0, len(itemid)):
			...
			itemid_url_history = "&itemids[{0}]={1}".format(i, itemid[i][0])
			zbx_img_url_itemids.append(itemid_url)
			zbx_img_url_history_itemids += itemid_url_history
		...
		zbx_img_url_history = self.server + "/history.php?&action=showgraph&graphtype=0" + zbx_img_url_history_itemids
		...
		res = requests.get(zbx_img_url, cookies=self.cookie, verify=self.verify, auth=requests.auth.HTTPBasicAuth(self.api_user, self.api_pass))
		...
		if res_code == 404:
			...
			return False
		self.img_url = zbx_img_url_history
...
def main():
	...
	for line in sys.argv[3].split('\n'):
		...
		if line.find('item1') == 0:
			...
			if host1tag.find('*UNKNOWN*') == -1:
				zdata.informsystem = host1tag
		...
		if line.find('period') == 0:
			image_period = line.split(';')[1]
		if line.find('<b>Severity:</b> Average') != -1:
			zdata.severity = 'high'
		if line.find('<b>Severity:</b> High') != -1:
			zdata.severity = 'high'
		if line.find('<b>Severity:</b> Disaster') != -1:
			zdata.severity = 'high'
		if line.find('<b>Severity:</b> Warning') != -1:
			zdata.severity = 'high'
...
#	image_period = zbxemail_settings.zbx_graph_period
	...
	problem_status = 'Waring'
	if zdata.triggerstatus == 'PROBLEM' and zdata.severity == 'high':	
		email.mail_head = '<p>'+body[0]+'<br>\n'+body[1]+'<br>\n'+body[3]+'</p><p style=\'background-color: #fca6a6\'>'+body[2]+'</p>'
		problem_status = 'High'
		email.mail_priority = 1
			
	if zdata.triggerstatus == 'PROBLEM' and zdata.severity != 'high':
		email.mail_head = '<p>'+body[0]+'<br>\n'+body[1]+'<br>\n'+body[3]+'</p><p style=\'background-color: #f7c922\'>'+body[2]+'</p>'
		problem_status = 'Waring'
		email.mail_priority = 0
	...
	zbx_image.graph_get(zdata.itemuniq, image_period, zdata.triggername, image_width, image_height, problem_status, zdata.hostid, zdata.event, zdata.trigger)
	...
	if zdata.informsystem:
		email.mail_subject = '!'+zdata.informsystem.rstrip()+'! '+sys.argv[2]
	...
	if zdata.history_types[0] == '0' or zdata.history_types[0] == '3':
		email.mail_head+='<p>'+'<a href="'+zbx_image.img_url+'">'+cid+'</a>'+'</p>'
